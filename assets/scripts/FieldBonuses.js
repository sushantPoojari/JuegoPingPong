...
NORD.Field.BonusContainer = function(field, config) {
  ...
  if (this.bonusType === 'TELEPORT1' || this.bonusType === 'TELEPORT2')
    this.radius = 25;
  if (this.bonusType === 'TELEPORT1') this.bg = Util.createSprite({
    parent: this,
    texture: 'portal1',
    aX: 0.5,
    aY: 0.5,
...
NORD.Field.BonusContainer.prototype = Object.create(NORD.Field.FieldObject.prototype);
...
NORD.Field.GravityWell.prototype = Object.create(NORD.Field.FieldObject.prototype);
...
NORD.Field.GravityWell.prototype.update = function() {
  ...
  balls.forEach(function(ball) {
    ...
    if (distance < maxDistance) {
      ...
      var rotAngle = Util.getRotAngle(ball.body.position.x, ball.body.position.y, _this10.x, _this10.y, moveAngle, _this10.gravityPower * gravityK);

      if (MultiplayerStarted) {
        if (IsHost) {

          ball.setMoveAnge(moveAngle + rotAngle);
          var seObj = new PP.ServerObject();
          seObj.ballBodyPositionX = ball.body.position.x;
          seObj.ballBodyPositionY = ball.body.position.y;
          seObj.paddlePositionY = RightPaddle.body.position.y;
          seObj.ballPositionX = ball.body.velocity.x;
          seObj.ballPositionY = ball.body.velocity.y;
        ...
        ball.setMoveAnge(moveAngle + rotAngle);
      // console.log('Gravity:', gravityK, ball, NORD.game.ballImpulse.counter++);
    }
...
NORD.Field.InvisibleArea.prototype = Object.create(NORD.Field.FieldObject.prototype);
...
NORD.Field.InvisibleArea.prototype.update = function() {
  ...
  var balls = this.field.getActiveBalls();
  balls.forEach(function(ball) {
    if (ball.body.position.x - ball.radius <= _this14.x + _this14.areaWidth / 2 && ball.body.position.x + ball.radius > _this14.x - _this14.areaWidth / 2) {
  ...
  this.tween1 = TweenMax.to(this.textReady, 6 / 30, {
    ...
    onComplete: function onComplete() {
      ...
      _this19.tween1 = TweenMax.to(_this19.textReady, 6 / 30, {
        ...
        onComplete: function onComplete() { // this.tween1 = TweenMax.to(this.textGo, 2/30, { y: 0, alpha: 1.0, delay: 0.2, ease: Power2.easeOut, onComplete: () =>
          // {
