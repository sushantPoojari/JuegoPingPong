...
Util.FrameTimer = function(framesDelay) {
  var curDelay = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : -1;
...
Util.rotatePointDeg = function(x, y, pivotX, pivotY, angle) {
  var p = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : null;
  if (p == null) p = {
    x: 0,
    y: 0
  };
...
//sushant
Util.checkCorrespondingRegion = function(region) {
  if (region == "asia")
    return "Asia";
  else if (region == "au")
    return "Australia";
...
Util.inbetweenRange = function(x1Val, x2Val) {
  return (x2Val >= (x1Val - 1) && x2Val <= (x1Val + 1))
};

Util.exceedsRange = function(y1Val, y2Val, maxRange) {
  var val = y1Val > y2Val ? y1Val - y2Val : y2Val - y1Val;
...
Util.isBetween = function(n, a, b) {
  return (n - a) * (n - b) <= 0
};

Util.memorySizeOf = function(obj) {
  var bytes = 0;
...
Util.getSlopePoint = function(x, x1, x2, y1, y2) {

  var slopePoint = (((x - x1) / (x2 - x1)) * (y2 - y1)) + y1;
  return slopePoint;
}

Util.calcHypotenuse = function(a, b) {
...
Util.getAngleBetween = function(angle1, angle2) {
  var v1 = (angle1 * Util.TO_RADIANS - angle2 * Util.TO_RADIANS) * Util.TO_DEGREES;
  var v2 = (angle2 * Util.TO_RADIANS - angle1 * Util.TO_RADIANS) * Util.TO_DEGREES;
...
Util.getDirectionVector = function(dir) {
  ...
  else if (dir == 'up_right') return {
    x: 1,
    y: -1
  };
  else if (dir == 'up_left') return {
    x: -1,
    y: -1
  };
  else if (dir == 'down_left') return {
    x: -1,
    y: 1
  };
  else if (dir == 'down_right') return {
    x: 1,
    y: 1
  };
  return null;
...
Util.createSprite = function(config) {
  var texture = null;
  if (config.atlas != undefined) texture = NORD.assetsManager.getTexture(config.atlas, config.texture);
  else texture = NORD.assetsManager.getTexture(config.texture);
  var sprite = new PIXI.Sprite(texture);
  if (config.parent != undefined) config.parent.addChild(sprite);
  if (config.aX != undefined && config.aY != undefined) sprite.anchor.set(config.aX, config.aY);
  ...
  if (config.scaleXY != undefined) sprite.scale.y = sprite.scale.x = config.scaleXY;
  if (config.rotation != undefined) sprite.rotation = config.rotation;
  if (config.visible != undefined) sprite.visible = config.visible;
  return sprite;
};

Util.createButton = function(name, parentPanel, container, positionType, x, y, width, height, tweenClick, soundClick, regularSkinData) {
  var btnConfig = null;
