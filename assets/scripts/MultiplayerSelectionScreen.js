...
NORD.MultiplayerSelectionPopup = function(config) {
  ...
  this.closeButton.addListener('button_click', function(data) {
    ...
    TweenMax.delayedCall(0.07 * 2, function() {
      _this5.hide("", function() {
        NORD.mainMenu.multiplayerSelectionPopup.hide();
      });
    });

  }, this);

  var textTitle = new PIXI.Text('PLAY A RANDOM OPPONENT', {
    font: '35px Snippet',
    fontSize: 17,
    fill: 'white',
    align: 'center'
  ...
  textTitle.position.x = 260;
  textTitle.position.y = 34;
  this.bg.addChild(textTitle);

  this.classicButton = Util.createButton('btn', this, null, '', -100, -110, 40, 40, NORD.game.tweenClickSimple, NORD.assetsManager.getAsset('CommonBtnMedium'), {
    texture: 'CommonBtnMedium',
    aX: 0.5,
    aY: 0.5,
    scaleX: 1,
    scaleY: 1
  ...
  this.classicButton.soundClick = NORD.assetsManager.getAsset('play_button')
  this.classicButton.addListener('button_click', function(data) {
    var _this5 = this;
    NORD.mainMenu.multiplayerSelectionPopup.hide("", function() {

      NORD.App.playerController.config.currentSelectedmode = NORD.MULTIPLAYER_GAME_MODE.CLASSIC;
  ...
  var textClassic = new PIXI.Text('CLASSIC', {
    font: '35px Snippet',
    fontSize: 17,
    fill: 'white',
    align: 'center'
  });
  textClassic.anchor.set(0.5);
  textClassic.position.set(-0, -0);
  this.classicButton.addChild(textClassic);

  this.actionButton = Util.createButton('btn', this, null, '', 100, -110, 40, 40, NORD.game.tweenClickSimple, NORD.assetsManager.getAsset('CommonBtnMedium'), {
    texture: 'CommonBtnMedium',
    aX: 0.5,
    aY: 0.5,
    scaleX: 1,
  ...
  this.actionButton.position.set(326, 373);
  this.actionButton.soundClick = NORD.assetsManager.getAsset('play_button')
  this.actionButton.addListener('button_click', function(data) {
    var _this5 = this;
    NORD.mainMenu.multiplayerSelectionPopup.hide("", function() {

      NORD.App.playerController.config.currentSelectedmode = NORD.MULTIPLAYER_GAME_MODE.ACTION;
  ...
  var textAction = new PIXI.Text('ACTION', {
    font: '35px Snippet',
    fontSize: 17,
    fill: 'white',
    align: 'center'
  });
  textAction.anchor.set(0.5);
  textAction.position.set(-0, -0);
  this.actionButton.addChild(textAction);

  var playCustomMatchTitle = new PIXI.Text('JOIN A CUSTOM MATCH', {
    font: '35px Snippet',
    fontSize: 17,
    fill: 'white',
    align: 'center'
  });
  playCustomMatchTitle.anchor.set(0.5);
  playCustomMatchTitle.position.x = 260;
  playCustomMatchTitle.position.y = 17;
  this.transparentBg.addChild(playCustomMatchTitle);

  this.createMatchButton = Util.createButton('btn', this, null, '', 0, 144, 40, 40, NORD.game.tweenClickSimple, NORD.assetsManager.getAsset('CommonBtnMedium'), {
    texture: 'CommonBtnBig',
    aX: 0.5,
    aY: 0.5,
    scaleX: 1,
    scaleY: 1
  });
  // this.createMatchButton.position.set(326,373);
  this.createMatchButton.soundClick = NORD.assetsManager.getAsset('play_button')
  this.createMatchButton.addListener('button_click', function(data) {
    var _this5 = this;
    //this.updateRandomName();

  }, this);

  var textcreateMatch = new PIXI.Text('CREATE CUSTOM MATCH', {
    font: '35px Snippet',
    fontSize: 17,
    fill: 'white',
    align: 'center'
  });
  textcreateMatch.anchor.set(0.5);
  textcreateMatch.position.set(-0, -0);
  this.createMatchButton.addChild(textcreateMatch);

  this.connectedRegion = new PIXI.Text('connected region : ' + Util.checkCorrespondingRegion(localStorage.getItem('savedServerRegion')), {
    font: '35px Snippet',
    fontSize: 17,
    fill: 'white',
    align: 'center'
  });
  this.connectedRegion.anchor.set(0.5);
  this.connectedRegion.position.x = 0;
  this.connectedRegion.position.y = 50;
  this.addChild(this.connectedRegion);
