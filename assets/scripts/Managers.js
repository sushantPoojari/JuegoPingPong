...
AssetsManager.prototype.loadAssets = function(assetsList, callback) {
  ...
  for (var i = 0; i < assetsList.length; i++) {
    ...
    if (assetInfo.type != undefined && assetInfo.type == 'AUDIO' || assetInfo.url.indexOf('.mp3') != -1 || assetInfo.url.indexOf('.ogg') != -1 || assetInfo.url.indexOf('.m4a') != -1 || assetInfo.url.indexOf('.wav') != -1) {
  ...
  function loadingComplete() {
    if (!(isAudioAssetsLoaded && isRegularAssetsLoaded && self.phase == 'loading')) return;
    self.phase = 'normal';
